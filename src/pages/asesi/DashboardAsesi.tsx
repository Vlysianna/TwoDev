			...
			if (response.data.success && response.data.data.length > 0) {
				...
				const mappedAssessments = response.data.data.map(
					(item: any, index: number) => ({
						...
						startDate: formatDateFromSchedule(
							item.assessment_schedule,
							"start"
						),
						endDate: formatDateFromSchedule(item.assessment_schedule, "end"),
						avatar: getInitials(
							item.assessment_schedule?.[0]?.assessor?.full_name || "Unknown"
						),
						avatarBg: getRandomColor(index),
						instructor:
							item.assessment_schedule?.[0]?.assessor?.full_name || "Asesor",
						role: "Asesor",
							...
							<div className="mb-6 bg-white rounded-lg shadow-sm p-4">
								<div className="flex flex-col md:flex-row md:items-center gap-4">
									<div className="flex-1">
										...
										<p className="text-sm text-gray-600">
											Daftar assessment yang tersedia untuk Anda
										</p>
									</div>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
									{filteredAssessments.length > 0 ? (
										filteredAssessments.map((assessment) => (
											<div
												key={assessment.id}
												className={`bg-white rounded-lg shadow-sm border-b-4 ${assessment.borderColor} hover:shadow-md transition-all duration-200 group`}
											>
												{/* Header Kartu */}
												<div className="p-4 border-b border-gray-100">
												...
												<div className="px-4 pb-4">
													<div className="flex items-center space-x-3">
														<div
															className={`w-8 h-8 ${assessment.avatarBg} rounded-full flex items-center justify-center text-white text-sm font-medium`}
														>
															{assessment.avatar}
														</div>
														<div className="flex-1">
