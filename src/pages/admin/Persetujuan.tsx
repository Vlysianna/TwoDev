...
const PersetujuanAdmin: React.FC = () => {
  ...
  const [selectedFilter, setSelectedFilter] = useState<
    "all" | "my-requests" | "my-approvals" | "backup-approvals"
  >("all");
  ...
  const fetchApprovals = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await api.get(`/approval/requests/scope/all`, {
      ...
      const list: ApprovalItem[] = (container as any[]).map((it: any) => {
        const requester =
          it?.requester || it?.requested_by || it?.user || it?.created_by || {};
        const fallbackResource =
          `${String(it?.target_table ?? it?.targetTable ?? "")}${it?.target_id || it?.targetId ? `#${it?.target_id ?? it?.targetId}` : ""}`.trim();
        return {
          id: Number(it?.id ?? it?.approval_id ?? it?.approvalId ?? 0),
          action: String(
            it?.action ?? it?.type ?? it?.operation ?? it?.verb ?? "",
          ),
          resource: String(
            it?.resource ??
              it?.target ??
          ...
          requester_admin_id:
            Number(
              it?.requester_admin_id ??
                it?.requesterAdminId ??
                requester?.admin_id ??
                0,
            ) || undefined,
          approver_admin_id:
            Number(it?.approver_admin_id ?? it?.approverAdminId ?? 0) ||
            undefined,
          backup_admin_id:
            Number(it?.backup_admin_id ?? it?.backupAdminId ?? 0) || undefined,
          approved_by:
            Number(it?.approved_by ?? it?.approvedBy ?? 0) || undefined,
          second_approver_admin_id:
            Number(
              it?.second_approver_admin_id ?? it?.secondApproverAdminId ?? 0,
            ) || undefined,
          status: it?.status,
      ...
      setError(
        err?.response?.data?.message || "Gagal memuat daftar persetujuan",
      );
    } finally {
      setLoading(false);
    }
  ...
  const applyFilters = (list: ApprovalItem[]) => {
    let effectiveFilter:
      ...
      | "my-requests"
      | "my-approvals"
      | "backup-approvals" = selectedFilter;
    if (canApprove === false) {
    ...
    if (effectiveFilter === "my-requests") {
      filtered = filtered.filter(
        (item) => item.requester_admin_id === currentAdminId,
      );
    } else if (effectiveFilter === "my-approvals") {
      filtered = filtered.filter(
        (item) => item.approver_admin_id === currentAdminId,
      );
    } else if (effectiveFilter === "backup-approvals") {
      filtered = filtered.filter(
        (item) => item.backup_admin_id === currentAdminId,
      );
  ...
  }, [selectedFilter, allItems, currentAdminId, canApprove]);
  ...
  const fetchCurrentAdmin = async () => {
    try {
      ...
      const found = list.find(
        (a: any) => Number(a?.user?.id ?? a?.user_id) === Number(user?.id),
      );
      if (found) {
        setCurrentAdminId(Number(found.id));
        const can =
          found?.can_approve === true ||
          found?.canApprove === true ||
          Number(found?.can_approve) === 1 ||
          Number(found?.canApprove) === 1;
  ...
  const resolveTargets = async (list: ApprovalItem[]) => {
    const keys = Array.from(
      new Set(
        list.map((it) => `${it.target_table || ""}:${it.target_id || ""}`),
      ),
    );
  ...
  const resolveRequesters = async (list: ApprovalItem[]) => {
    const ids = Array.from(
      new Set(list.map((it) => it.requester_admin_id).filter(Boolean)),
    ) as number[];
    ...
    await Promise.all(
      userIds.map(async (userId: unknown) => {
        try {
  ...
  const act = async (id: number, type: "approve" | "reject") => {
    try {
      ...
      setError(null);
      console.log(
        `🔍 Verification - Starting approval: ID=${id}, Type=${type}`,
      );
      const res = await api.post(`/approval/${id}/${type}`);
      console.log(`🔍 Verification - Approval response:`, res?.data);

      if (res?.data?.success !== false) {
        toast.show({
          title: "Berhasil",
          description:
            type === "approve" ? "Permintaan disetujui" : "Permintaan ditolak",
          type: "success",
        });
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-auto p-6">
          ...
          <div className="text-sm text-gray-500 mb-2">
            Dashboard / Persetujuan Penghapusan
          </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  {canApprove !== false && (
                    <option value="backup-approvals">Backup Persetujuan</option>
                  )}
                </select>
              </div>
              <button
              ...
              <table className="min-w-full table-auto text-left">
                <thead>
                  <tr className="bg-gradient-to-r from-orange-500 to-orange-600 text-white text-xs sm:text-sm">
                    <th className="px-2 py-3 sm:px-4 lg:px-6 font-medium rounded-tl-xl text-left">
                      Pemohon
                    </th>
                    <th className="px-2 py-3 sm:px-4 lg:px-6 font-medium text-left">
                      Target
                    </th>
                    <th className="px-2 py-3 sm:px-4 lg:px-6 font-medium text-left">
                      Catatan
                    ...
                    <th className="px-2 py-3 sm:px-4 lg:px-6 text-center font-medium">
                      Setujui
                    </th>
                    <th className="px-2 py-3 sm:px-4 lg:px-6 text-center font-medium rounded-tr-xl">
                      Status
                    </th>
                  </tr>
                </thead>
                    ...
                    items.map((it) => {
                      ...
                      const isCurrentAdminApprover =
                        ...
                        (it.approver_admin_id === currentAdminId ||
                          it.backup_admin_id === currentAdminId);

                      const canApproveRow =
                        isPending &&
                        isCurrentAdminApprover &&
                        (Number(it.requester_admin_id) !== currentAdminId ||
                          selectedFilter === "backup-approvals");

                      return (
                        <React.Fragment key={it.id}>
                          <tr
                            className={`group transition-colors ${it.id % 2 === 0 ? "bg-gray-50/50" : "bg-white"} hover:bg-orange-50/50 cursor-pointer`}
                          ...
                          >
                            <td className="px-2 py-3 sm:px-4 lg:px-6 break-words whitespace-normal">
                              <div className="flex items-center gap-2">
                                <ChevronDown
                                  size={14}
                                  className={`transition-transform ${expandedIds.includes(it.id) ? "rotate-180" : ""}`}
                                />
                                <span>
                                  {requesterLabels[
                                    it.requester_admin_id || 0
                                  ] ||
                                    it.requester?.full_name ||
                                    it.requester?.email ||
                                    "-"}
                                </span>
                              </div>
                            ...
                            <td
                              className="px-4 py-4 lg:px-6 text-center"
                            ...
                            <td className="px-2 py-3 sm:px-4 lg:px-6 text-center">
                              <span
                                className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${statusDisplay.className}`}
                              >
                                {statusDisplay.icon}
                                {statusDisplay.text}
                              </span>
                          ...
                          {expandedIds.includes(it.id) && (
                            <tr
                              className={`${it.id % 2 === 0 ? "bg-gray-50/30" : "bg-white"}`}
                            >
                              <td
                                colSpan={6}
                                className="px-4 lg:px-6 py-3 text-sm"
                              >
                                ...
                                "approved" ? (
                                  <div className="rounded-md border border-green-200 bg-green-50 text-green-800 px-3 py-2 inline-flex items-center gap-2">
                                    ...
                                    <span className="font-medium">
                                      {approverLabels[it.approved_by || 0] ||
                                        "-"}
                                    </span>
                                  </div>
                                ) : String(it.status || "").toLowerCase() ===
