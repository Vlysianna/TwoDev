...
const DashboardAsesor: React.FC = () => {
  ...
  interface Schedule {
    start_date?: string;
    end_date?: string;
    assessment?: { occupation?: AssessmentOccupation };
  ...
  const [programFilters, setProgramFilters] = useState<Array<{ code: string; name: string; count: number; color: string }>>([]);
          ...
          (sch.schedule_details || []).forEach((detail) => {
            if (assessor && detail.assessor?.id === assessor.id) {
              assigned.push({
                ...
                avatar: getInitials(detail.assessor?.full_name || user.email || 'AS'),
                avatarBg: 'bg-[#60B5FF]',
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center space-x-2 mt-6">
              <button
                ...
                className={`px-4 py-2 rounded-md ${selectedProgram === 'ALL' ? 'bg-blue-500 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>
                Semua ({cards.length})
              </button>
              {programFilters.map((p) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-md ${selectedProgram === p.code ? `${p.color} text-white` : 'text-gray-600 hover:bg-gray-100'}`}>
                  {p.code} ({p.count})
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {!loading && !error && visibleCards.length === 0 && (
              <div className="lg:col-span-3 text-center py-12">
                <p className="text-gray-600">Belum ada tugas asesmen untuk Anda.</p>
              </div>
            )}

            {!loading && !error && visibleCards.length > 0 && visibleCards.map((okupasi) => {
              return (
                <div
                  key={okupasi.id}
